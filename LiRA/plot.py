...
parser.add_argument('--output_folder', type=str, default='attack_outputs',
                    help='output folder for storing the attack outputs')
...
def generate_ours_offline(keep, scores, check_keep, check_scores, in_size=100000, out_size=100000,
    ...
    for j in range(scores.shape[1]):
        dat_in.append(scores[keep[:, j], j,
                      :])  # take all the in_model for that particular sample (keep is used as a bool to identify in_model)
        dat_out.append(scores[~keep[:, j], j, :])
...
def do_plot(fn, legend='', metric='auc', sweep_fn=sweep, **plot_kwargs):
    ...
    prediction, answers = fn(shadow_labels,
                             ...
                             test_labels,
                             # labels of the testing data, including both members and non-members (for evaluating the attack)
    ...
    if (args.save_tag != None):
        ...
        np.save(os.path.join(args.output_folder, 'lira-%s-%s.npy' % (legend, args.save_tag)),
                np.r_[answers, prediction * -1])
