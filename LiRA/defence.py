...
print(
    'tr len %d | mia_members tr %d val %d te %d | mia_nonmembers tr %d val %d te %d | ref len %d | val len %d | test len %d | attack te len %d | remaining data len %d' %
...
best_model = resnet18(num_classes=10, weights=None).cuda()
...
return_outputs, return_labels = _model_predictions(best_model, all_shadow_data_tensor, all_shadow_label_tensor,
                                                   batch_size=256)
